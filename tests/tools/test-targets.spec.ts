describe("checkForConsistency", () => {
  const createTestCase = (id: string, overrides?: Partial<PushTestTargetTestCase>): PushTestTargetTestCase => ({
    id,
    version: "1",
    tagNames: [],
    runStatus: "ON",
  ...
  it("throws when duplicate test case ids exist", () => {
    ...
    expect(() => checkForConsistency(testCases)).toThrow(/Duplicate test case id tc-1/);
  });
  ...
  it("throws when dependencyId references a non-existing test case", () => {
    ...
    expect(() => checkForConsistency(testCases)).toThrow(/Test case tc-2 depends on non existing test case tc-999/);
  });
  ...
  it("throws when teardownId references a non-existing test case", () => {
    ...
    expect(() => checkForConsistency(testCases)).toThrow(/Test case tc-2 depends on non existing teardown tc-999/);
  });
  ...
  it("throws when there is a direct cyclic dependency (A -> A)", () => {
    ...
    expect(() => checkForConsistency(testCases)).toThrow(/Cyclic dependency detected.*tc-1/);
  });
  ...
  it("throws when there is a two-node cycle (A -> B -> A)", () => {
    ...
    expect(() => checkForConsistency(testCases)).toThrow(/Cyclic dependency detected/);
  });
