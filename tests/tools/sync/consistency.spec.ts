...
describe("checkForConsistency", () => {
    ...
    it("passes with complex dependency chains", () => {
        const testCases = [
            ...
            createMockSyncTestCase({ id: "test-1", dependencyId: "login", teardownId: "teardown" }),
            createMockSyncTestCase( {id: "test-2", dependencyId: "login", teardownId: "teardown" }),
        ];
        expect(() => checkForConsistency(testCases)).not.toThrow();
    });
    ...
    it("passes when dependencyId and teardownId are undefined", () => {
        const testCases = [
            createMockSyncTestCase( {id: "tc-1", dependencyId: undefined, teardownId: undefined }),
        ];
        expect(() => checkForConsistency(testCases)).not.toThrow();
    });
