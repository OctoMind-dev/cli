...
export type ExportTestCaseOptions = {
  testTargetId: string;
  testCaseId: string;
  format: "yaml" | "json";
};
...
export const exportTestCase = async (
  options: ExportTestCaseOptions & ListOptions,
): Promise<void> => {
  const { data, error } = await client.GET(
    "/apiKey/v3/test-targets/{testTargetId}/test-cases/{testCaseId}/export",
    {
      params: {
        path: {
          testTargetId: options.testTargetId,
...
export type ImportTestCaseBody = {
  body: paths["/apiKey/v3/test-targets/{testTargetId}/test-cases/import"]["post"]["requestBody"]["content"]["application/json"];
  testTargetId: string;
  input: string;
};

export const importTestCase = async (
  options: ImportTestCaseBody & ListOptions,
): Promise<void> => {
  let input = "";
  if (options.input === "-") {
    const chunks: Buffer[] = [];
  ...
  try {
    ...
    options.body = yaml.parse(input);
  }

  const { data, error } = await client.POST(
    "/apiKey/v3/test-targets/{testTargetId}/test-cases/import",
    {
      params: {
        path: {
          testTargetId: options.testTargetId,
