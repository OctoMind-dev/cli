...
export type listEnvironmentsOptions = paths["/apiKey/v2/test-targets/{testTargetId}/environments"]["get"]["parameters"]["path"];
export type executeTestsBody = components["schemas"]["TestTargetExecutionRequest"];
export type gettestReportParams = paths["/apiKey/v2/test-targets/{testTargetId}/test-reports/{testReportId}"]["get"]["parameters"]["path"];
...
export type EnvironmentRequest = paths["/apiKey/v2/test-targets/{testTargetId}/environments"]["post"]["requestBody"]["content"]["application/json"] & { testTargetId: string };
export type Environment = components["schemas"]["EnvironmentResponse"];
export type UpdateEnvironmentOptions = paths["/apiKey/v2/test-targets/{testTargetId}/environments/{environmentId}"]["patch"]["requestBody"]["content"]["application/json"];
...
export const listEnvironments = async (
  ...
  const { data, error } = await client.GET("/apiKey/v2/test-targets/{testTargetId}/environments", {
    params: {
...
export const getTestReport = async (
  ...
  const { data, error } = await client.GET("/apiKey/v2/test-targets/{testTargetId}/test-reports/{testReportId}", {
    params: {
      path: { testTargetId: options.testTargetId, testReportId: options.testReportId },
    } });
...
export const registerLocation = async (
  options:{ address: string, name: string, username: string, password: string, json?: boolean },
): Promise<void> => {

  const { data, error } = await client.PUT("/apiKey/v1/private-location/register", {
    body: {
      name: options.name,
      registrationData: {
...
export const unregisterLocation = async (
  options: { name: string, json?: boolean }
   ...
   const { data, error } = await client.PUT("/apiKey/v1/private-location/unregister", { 
    body: {
      name: options.name,
    }
   }
...
export const createEnvironment = async (
  ...
  const { data, error } = await client.POST("/apiKey/v2/test-targets/{testTargetId}/environments", {
    params: {
      path: { testTargetId: options.testTargetId },
    },
    body: {
...
export const updateEnvironment = async (
  options: UpdateEnvironmentOptions & { testTargetId: string, environmentId: string, json?: boolean }
): Promise<void> => {

  const { data, error } = await client.PATCH("/apiKey/v2/test-targets/{testTargetId}/environments/{environmentId}", {
    params: {
      path: { testTargetId: options.testTargetId, environmentId: options.environmentId },
    },
    body: {
      name: options.name,
      discoveryUrl: options.discoveryUrl,
...
export const deleteEnvironment = async (
  options: { testTargetId: string, environmentId: string, json?: boolean, apiKey?: string }
): Promise<void> => {
  const { data, error } = await client.DELETE("/apiKey/v2/test-targets/{testTargetId}/environments/{environmentId}", {
    params: {
      path: { testTargetId: options.testTargetId, environmentId: options.environmentId },
    },
  });
